...
public class TextureConfig implements ResourcesType {


    @Override
    public ResourcesType init() {
        JsonReader json = new JsonReader();
        JsonValue jsonData = json.parse(Gdx.files.internal("config/type/texture.json"));

        for(JsonValue category = jsonData.getChild("texture"); category != null; category = category.next()) {
